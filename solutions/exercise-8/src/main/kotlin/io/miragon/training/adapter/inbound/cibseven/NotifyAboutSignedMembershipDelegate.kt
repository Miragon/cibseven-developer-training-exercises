package io.miragon.training.adapter.inbound.cibseven

import io.miragon.training.application.port.inbound.NotifyAboutSignedMembershipUseCase
import io.miragon.training.domain.MembershipId
import org.cibseven.bpm.engine.delegate.DelegateExecution
import org.springframework.stereotype.Component
import java.util.UUID

@Component
class NotifyAboutSignedMembershipDelegate(
    private val useCase: NotifyAboutSignedMembershipUseCase,
) : BaseDelegate() {

    override fun executeTask(execution: DelegateExecution) {
        val membershipId = execution.getVariable("membershipId") as String
        log.debug { "Received task to notify about signed membership: $membershipId" }
        useCase.notifyAboutSignedMembership(MembershipId(UUID.fromString(membershipId)))
    }
}
